...
## Define variable branches

```{r define_outliers}
# the exclusion criteria will be implemeted using filtering branches
death_outliers <- variable_branch(
  TRUE, 
  name != 'Katrina', 
  !(name %in% c('Katrina', 'Audrey'))
)
damage_outliers <- variable_branch(
  TRUE, 
  name != 'Sandy', 
...
mv <- mv %>%
  add_variable_branch(death_outliers, damage_outliers) %>%
  execute_multiverse()
summary(mv)
```

```{r define_variables}
femininity <- variable_branch(
  female,
  masfem
)
damage <- variable_branch(
  dam,
  log(dam)
)
mv <- mv %>%
  add_variable_branch(femininity, damage) %>%
  execute_multiverse()
summary(mv)
```
  ...
  mv, {
    model <- glm(
      ...
      #   'none' ~ NULL
        ...
        'linear' ~ gaussian,
        'poisson' ~ poisson
      ), data = data %>% filter(death_outliers, damage_outliers)
    )
