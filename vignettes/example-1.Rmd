...
```{r hist_base, fig.caption="Histogram of skin tones variale based on the first definition.", fig.width=4, fig.height=4, warning=FALSE}
library(ggplot2)
...
#   (rater1 + rater2)/2,
#   ifelse(rater1 > rater2, rater1, rater2),
#   ifelse(rater1 > rater2, rater2, rater1),
#   rater1,
#   rater2
# )
# ```
...
```{r compare_universe, fig.caption="Density lines overlaid on a single plot.", fig.width=4, fig.height=4, warning=FALSE}
branched %>%
...
```{r compare_universe_hist, fig.caption="Histograms on a facet grid.", fig.width=4, fig.height=4, warning=FALSE}
branched %>%
  ggplot(mapping = aes(x = skin_tone, fill = universe)) +
  geom_histogram(position = 'dodge', bins = 10) +
  labs(title = 'Histogram of player skin tones.',
       y = 'Count', x='Skin Tone') +
  # generate histograms for each universe and place along a column.
  facet_grid(. ~ universe)
```
