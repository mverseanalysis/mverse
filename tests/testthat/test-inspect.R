...
test_that("summary.lm_mverse() outputs coefficient estimates with 95% confidence intervals by default.", {
  ...
  smanu <- do.call(rbind, list(
    tibble(term = "(None)", estimate = NA, std.error = NA,
  ...
  smverse <- summary(mv) %>% select(-c(universe, model_spec_branch))
...
test_that("summary.lm_mverse(., conf.int = FALSE) outputs coefficient estimates without any confidence intervals.", {
  ...
  smverse <- summary(mv, conf.int = FALSE) %>% select(-c(universe, model_spec_branch))
  expect_identical(smverse, smanu)
})
...
test_that("summary.lm_mverse(output = 'de') outputs degrees of freedom.", {
  ...
  mv <- mverse(mydf) %>%
    ...
    lm_mverse()
  smanu <- do.call(rbind, list(
    tibble(p = NA, n.minus.p = NA, p.star = NA),
...
test_that("summary.lm_mverse(output = 'r') outputs R squared values.", {
  ...
  mv <- mverse(mydf) %>%
    ...
    lm_mverse()
  smanu <- do.call(rbind, list(
    tibble(r.squared = NA, adj.r.squared = NA),
    ...
    as.data.frame(t(c(summary(lm(y ~ x1 * x2, data = mydf))$r.squared,
      ...
      rename(r.squared = V1, adj.r.squared = V2)))
  smverse <- summary(mv, output = "r") %>% select(-c(universe, model_spec_branch))
  smverse2 <- summary(mv, output = "r.squared") %>% select(-c(universe, model_spec_branch))
  expect_identical(smverse, smanu)
  expect_identical(smverse2, smanu)
})
...
test_that("summary.lm_mverse(output = 'f') outputs F statistics.", {
  ...
  mv <- mverse(mydf) %>%
    ...
    lm_mverse()
  smanu <- do.call(rbind, list(
    tibble(fstatistic = NA, numdf.f = NA, dendf.f = NA),
    tibble(fstatistic = NA, numdf.f = NA, dendf.f = NA),
    ...
    as.data.frame(t(summary(lm(y ~ x1 * x2, data = mydf))$fstatistic)) %>%
      rename(fstatistic = value, numdf.f = numdf, dendf.f = dendf)))
  smverse <- summary(mv, output = "f") %>% select(-c(universe, model_spec_branch))
  smverse2 <- summary(mv, output = "fstatistic") %>% select(-c(universe, model_spec_branch))
  expect_identical(smverse, smanu)
  expect_identical(smverse2, smanu)
})
